...
class MessageCQ(MessageBase):
    ...
    def __init__(
        ...
        super().__init__(
            message_info=message_info, message_segment=None, raw_message=None
        )
    ...
    def _parse_message(self, message: str, reply_message: Optional[Dict] = None) -> Seg:
        ...
        for code_item in cq_code_dict_list:
            message_obj = cq_code_tool.cq_from_dict_to_class(
                code_item, msg=self, reply=reply_message
            )
...
class MessageSendCQ(MessageCQ):
    ...
    def __init__(self, data: Dict):
        ...
        self.bot_message = self._generate_bot_message()

    def _generate_bot_message(
        self,
    ) -> str:
        ...
        if self.message_segment.type == "seglist":
            for seg in self.message_segment.data:
                segments.append(self._seg_to_msg_seg(seg))
            return Message(segments)

        return self._seg_to_msg_seg(self.message_segment)

    def _seg_to_msg_seg(self, seg: Seg) -> MessageSegment:
        """将单个Seg对象转换为CQ码字符串"""
        if seg.type == "text":
            return MessageSegment.text(seg.data)
        elif seg.type == "image":
            return MessageSegment.file_image(base64.b64decode(seg.data))
        elif seg.type == "emoji":
            return MessageSegment.file_image(base64.b64decode(seg.data))
        elif seg.type == "at":
            return MessageSegment.mention_user(seg.data)
        elif seg.type == "reply":
            return MessageSegment.reference(seg.data)
        else:
            return MessageSegment.text(seg.data)
